...
% Modified to work on different pyramid steepness -owang

function res = reconSCFpyr_scale(pyr, indices, levs, bands, twidth, scale, nScales)
...
pind = indices;
Nsc = round(log2(pind(1,1)/pind(end,1))/log2(1/scale)); %new
Nsc = nScales;
Nor = (size(pind,1)-2)/Nsc;

pyrTmp = [];
ind1 = pyrBandIndices(pind,1);
...
ind2 = pyrBandIndices(pind,nband+1);
pyr = [pyr(ind1); pyrTmp; pyr(ind2)];
clear pyrTmp

res = reconSFpyr_scale(pyr, indices, levs, bands, twidth, scale);
