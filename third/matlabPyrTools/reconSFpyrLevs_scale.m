...
function resdft = reconSFpyrLevs_scale(pyr,pind,log_rad,Xrcos,Yrcos,angle,nbands,levs,bands,scale)
...
if any(levs > 1)
  ...
  %lodims = ceil((dims-0.5)*scale);
  lodims = pind(1+bands(end),:); %new
  ...
  if  (size(pind,1) > lo_ind)
    nresdft = reconSFpyrLevs_scale( pyr(1+sum(prod(pind(1:lo_ind-1,:)')):size(pyr,1)),...
	pind(lo_ind:size(pind,1),:), ...
	nlog_rad, Xrcos, Yrcos, nangle, nbands,levs-1, bands, scale);
  else
